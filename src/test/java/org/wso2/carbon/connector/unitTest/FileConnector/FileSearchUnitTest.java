...
import org.wso2.carbon.connector.FileSearch;
...
/**
 * Unit test class for FileSearch method
 */
public class FileSearchUnitTest {
    ...
    @Test
    public void testFileSearchExist() throws Exception {
        ...
        templateContext.getMappedValues().put("recursiveSearch", "true");
        ...
        fileSearch.connect(ctx);

        Assert.assertNotNull(ctx.getEnvelope().getBody().getFirstElement());
    }

    @Test
    public void testFileSearchNotExist() throws Exception {
        TemplateContext templateContext = new TemplateContext("fileConnector", null);
    ...
    @Test
    public void testFileSearchFolderNotExist() throws Exception {
        TemplateContext templateContext = new TemplateContext("fileConnector", null);
        templateContext.getMappedValues().put("source", getFilePath("in/search"));
        templateContext.getMappedValues().put("filePattern", "[a-zA-Z][a-zA-Z]*.(txt|xml)");
        templateContext.getMappedValues().put("recursiveSearch", "true");
        templateContext.getMappedValues().put("setTimeout", "");
        templateContext.getMappedValues().put("setPassiveMode", "");
        ...
        System.out.print(ctx.getEnvelope().getBody().getFirstElement());
        Assert.assertNotNull(ctx.getEnvelope().getBody().getFirstElement());
    }
